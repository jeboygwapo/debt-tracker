...
async def test_admin_blocked_for_non_admin(client):
    # Login as non-admin — create one first
    from app.db.base import AsyncSessionLocal
    from app.db.crud import create_user, get_user_by_username, delete_user
    ...
    r = await client.post("/login", data={"username": "nonadmin_test", "password": "NonAdminPass123!"})
    r2 = await client.get("/admin")
    # redirects to / (not admin)
...
async def test_admin_create_and_delete_user(authed_client):
    ...
    # Delete
    r2 = await authed_client.post(f"/admin/users/{user_id}/delete")
...
async def test_admin_cannot_delete_self(authed_client):
    ...
    import re
    r = await authed_client.get("/admin")
    match = re.search(r'id=(\d+).*?testadmin|testadmin.*?id=(\d+)', r.text, re.DOTALL)
    # just check the page still loads after attempt
    r2 = await authed_client.post("/admin/users/1/delete")
    assert r2.status_code == 200
